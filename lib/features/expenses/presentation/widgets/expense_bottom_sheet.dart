...
class _ExpenseBottomSheetState extends ConsumerState<ExpenseBottomSheet> {
  ...
  Future<void> _save() async {
    if (!_formKey.currentState!.validate() || _selectedCategory == null || _isLoading) return;
    ...
    try {
      if (widget.expense == null) {
        ...
        await addUseCase.execute(expense);
      } else {
        final updateUseCase = ref.read(updateExpenseUseCaseProvider);
        await updateUseCase.execute(expense);
      }
  ...
  Future<void> _delete() async {
    ...
    try {
      ...
      await deleteUseCase.execute(widget.expense!.id!);
